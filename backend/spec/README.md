...
│   ├── spec/                  # 规格文档（含本 README）
│   ├── sql/                   # **所有手工建表 DDL 的单一汇总点**（v0.6.0 起新增；详见 §3.8）
│   │   └── dev_sql.sql
│   ├── .env.example
│   └── requirements.txt
│
...
## 4. 数据库表手工建表规范（v0.6.0 新增）

> **历史上下文（v0.6.0 之前）**：spec/email_notification.md 第 88 行曾预警「仓库未引入 Alembic；首次启动依赖 `Base.metadata.create_all()` ……已有 MySQL 实例不会自动加列」。该预警**没有被严格落实** —— v0.6.0 新增 3 张表既未走 create_all，也未手工建表，结果首次部署到 MySQL `sparkmemo` 时 500 报 `Table 'sparkmemo.cross_table_fill_jobs' doesn't exist`。本节正式确立项目级硬约束。

### 4.1 硬约束

| 项 | 规则 |
|---|---|
| **建表方式** | **所有 MySQL 表手工 SQL 建表**；app lifespan **不**调用 `Base.metadata.create_all` |
| **SQL 集中点** | 全部 DDL 写在 **`backend/sql/dev_sql.sql`** 单一文件**（严禁拆分到多个 .sql 文件） |
| **幂等性** | 全部 `CREATE TABLE IF NOT EXISTS`，可重复执行 |
| **列变更** | 走 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS ...`（MySQL 8.0+）或显式函数（如 `_ensure_email_config_columns`），不依赖 ORM 自动 |
| **测试环境** | `tests/conftest.py` 保留 `Base.metadata.create_all(engine)` 兜底（SQLite），与生产 MySQL 解耦 |
| **新增模块流程** | 新加 ORM 类 → 立即把对应 `CREATE TABLE IF NOT EXISTS` 追加到 `dev_sql.sql` → PR 提交前必带 |

### 4.2 dev_sql.sql 组织约定
...
-- ============================================================
-- 未来模块 / 老模块 backfill 追加此处
-- ============================================================
```

- **以模块 + 版本号分块**：每块前一行 `=` 分隔的注释标题
- **DDL 内带 COMMENT**：每张表 / 字段必须 `COMMENT='...'`；与 ORM 字段 docstring 保持一致
- **FK 必须显式命名** + `ON DELETE CASCADE` 行为与 ORM `cascade="all, delete-orphan"` 对齐

### 4.3 执行方式

```bash
# MySQL 命令行（推荐）
mysql -u <user> sparkmemo < backend/sql/dev_sql.sql

# 或 pymysql（如果没装 mysql 客户端）
& "D:\anaconda3\envs\dev_env\python.exe" -c "
import pymysql
conn = pymysql.connect(host='127.0.0.1', user='root', password='...', database='sparkmemo')
sql = open('backend/sql/dev_sql.sql', encoding='utf-8').read()
...
- **测试环境（SQLite）**：`tests/conftest.py:53` 仍调用 `Base.metadata.create_all(engine)`；不依赖本文件
- **测试环境**也可选读取 `dev_sql.sql` 中 SQLite 兼容的子集，与生产同源（**未来可优化项**，不在本轮范围）
- CI / 部署流水线默认**不**自动跑 `dev_sql.sql`；运维升级时手动执行

### 4.5 关联文档

- v0.6.0 首次落地：[`./cross_table_fill.md`](./cross_table_fill.md) §11 v0.6.0.0.2（缺表 hot fix 修订记录）
- 上一次冷启动相关讨论：[`./email_notification.md`](./email_notification.md) §「数据库可移植性约束」章节
