...
SparkMemo 是单用户的本地化任务提醒 Web 应用。用户管理多家公司下的多个项目，并为每个项目维护任务（含自定义类型、起止提醒日期）。到截止日期前由系统每天提醒一次，逾期 3 天的任务自动标记为完成。前端是对接 FastAPI 后端的单页应用，承载所有 UI 交互，不持有持久化数据，不做业务计算（提醒计划生成、逾期判断等均由后端完成）。
...
| 字段级硬约束 | `remind_start_at <= due_at`；`company_id` / `project_id` 必填且对应记录存在 |
...
┌───────────────────────────────────────────────────────────────┐
│  顶部 Header（48px）                                          │
│   - 左侧：Logo / 应用名「SparkMemo」                          │
│   - 中部：tab 切换（全部 / 今日待提醒）                       │
│   - 右侧：导航（设置占位）                                    │
├───────────────────────────────────────────────────────────────┤
│                                                               │
│  主内容区 Main（自适应，最大宽 1440px，左右内边距 24px）       │
│                                                               │
├───────────────────────────────────────────────────────────────┤
│  底部 Footer（占位，v0.1 不渲染内容）                          │
└───────────────────────────────────────────────────────────────┘
```

| 项 | 规范 |
|----|------|
| 顶部 Header 高度 | 48px，固定 |
| 主内容区最大宽度 | 1440px；超过 1440px 居中显示 |
